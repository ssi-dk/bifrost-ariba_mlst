...
def run_sample(args: object):
    ...
    if len(sample) == 0:
        ...
        print(f"snakemake -s /bifrost/{COMPONENT['name']}/src/pipeline.smk --config sample_id={str(sample[0]['_id'])} component_id={str(component[0]['_id'])}")
        try:
            process: subprocess.Popen = subprocess.Popen(
                f"snakemake -s /bifrost/{COMPONENT['name']}/src/pipeline.smk --config sample_id={str(sample[0]['_id'])} component_id={str(component[0]['_id'])}",
                stdout=sys.stdout,
                stderr=sys.stderr,
                shell=True
            )
